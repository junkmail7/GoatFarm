...
#@onready var draw_surface : Surface# get_node("/root/blood_scene/surface")
func _physics_process(delta: float) -> void:
	if(!is_colliding): # in the air
		...
		#var local_pos = Surface.global_to_local(global_position)
		Surface.draw_blood(position, 2.0)  # draw blood to surface
