...
@onready var ai_eyes : RayCast2D = $ai_eyes
@onready var ai_eyes_up : RayCast2D = $ai_eyes_up
@onready var ai_eyes_left : RayCast2D = $ai_eyes_left
...
var move_direction = Vector2.RIGHT  # Set the initial position
...
func _physics_process(delta):
	...
	var direction_to_player = Global.p1_lokaysh - global_position
	#print(direction_to_player)

	# Check if the player is above the AI and within a certain X distance
	if !(direction_to_player.y < -100 and abs(direction_to_player.x) < 200):
#		# Jump
#		#velocity.y = -100.0  # Define jump_impulse for jump strength
#	else:
	...
	if character_state_machine.check_if_can_move():
		velocity.x = move_toward(velocity.x, SPEED * move_direction.x, acceleration * delta)
	else:
		# Your existing smartguy logic
		if smartguy == true:
			if face_dir == true:
